...
— `init` сканирует проект и объявляет каждый каталог с Go-кодом как компонент (`Path`), а каждый существующий импорт становится правилом `Use` — свежий каркас зеркалит код как есть и сразу проходит проверку. Ужесточить правило — значит удалить строку `Use` и посмотреть, какие нарушения всплывут.
