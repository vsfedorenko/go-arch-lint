...
> Проще начать с нуля: `rm -rf .go-arch-lint && go-arch-lint init` сгенерирует
...
Все три примера — живые проекты в [examples/](examples/) (basic — слои, ddd —
bounded contexts, hexagonal — порты и адаптеры), и CI проверяет каждый:
job `examples` прогоняет `go-arch-lint check` по всем трём на каждом PR.
...
- **Go workspaces (`go.work`).** Проект с корневым `go.mod` работает полностью — включая соседние модули-члены workspace: `init` исключает их из архитектуры по умолчанию, а при желании их можно объявить компонентами (`Path("two/y")`) и писать кросс-модульные правила `Use`. Модуль-член может иметь любой собственный module path (`example.com/y`, не обязательно `example.com/root/two/y`) — импорты членов классифицируются как project-код, а не vendor. Workspace из модулей-сиблингов без корневого `go.mod` пока не поддерживается — CLI подскажет это при запуске.
...
Создаёт `.go-arch-lint/` с тремя файлами. Спека и раннер разделены: `arch.go` —
ваш файл, `main.go` — служебный, его трогать не нужно:

```go
// arch.go — здесь вы описываете архитектуру
package main
...
var build = Spec(func() {
	// Каждый каталог с Go-кодом объявлен, и каждый существующий
	// импорт разрешён: каркас зеркалит код как есть. Ужесточайте
	// правила Use по мере роста архитектуры:
	//
	//     domain := Path("internal/domain")
...
```go
// main.go — сгенерирован, не редактируйте: пробрасывает флаги CLI в проверку
package main
...
— `init` сканирует проект и объявляет каждый каталог с Go-кодом как компонент (`Path`), каждый внутренний импорт становится правилом `Use`, а каждая сторонняя библиотека — декларацией `Vendor` + `Use` — свежий каркас зеркалит код как есть и сразу проходит проверку. Ужесточить правило — значит удалить строку `Use` и посмотреть, какие нарушения всплывут.
...
### Готовые шаблоны архитектур

Не хотите начинать с чистого листа — возьмите пример из каталога [`examples/`](examples/): скопируйте `.go-arch-lint/` из [basic](examples/basic/) (слои), [ddd](examples/ddd/) (bounded contexts) или [hexagonal](examples/hexagonal/) (порты и адаптеры) в свой проект и поправьте пути. Каждый пример — законченный проект со спекой на актуальном DSL.

Флаг `--recipe` существовал в v1/v2 и удалён в v3: `init` больше не принимает ни его, ни других неизвестных флагов — молчаливый откат к дефолтной спеке был бы хуже ошибки.
...
Код 2 означает «проверка не выполнялась» — чините конфигурацию, а не код.
...
- `Path("a/b")` — каталог с Go-кодом, то есть компонент. `Path("a/b/**")` — всё
