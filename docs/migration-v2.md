...
- Логика линтера (assembler, checker, renderer) не изменилась. Изменился только
  формат конфигурации.
- Коды возврата следуют конвенции линтеров: `0` — нарушений нет, `1` — найдены
  нарушения, `2` — ошибка конфигурации (arch.go не компилируется, проект
  не читается). См. `archlint.ExitCode`.
