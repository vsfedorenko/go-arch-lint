...
Tests use [testify](https://github.com/stretchr/testify) exclusively:
`assert`/`require` for every check — no hand-written `if got != want
{ t.Fatal }` comparisons. `require` for failures that make the rest of
the test meaningless (errors, lengths), `assert` for informational
checks that can accumulate. Interface doubles are generated by
[mockery](https://github.com/vektra/mockery) with the repo's default
`.mockery.yml` config (testify template, mocks in-package as
`mocks_test.go`); mocks are driven strictly via `EXPECT()`:

```bash
make mocks    # regenerate after changing an interface
```

Mocks are committed. When you add an interface that needs a double in
tests, list it under `packages:` in `.mockery.yml` and run `make mocks`.
